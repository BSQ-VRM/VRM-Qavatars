...
namespace VRMQavatars {
    ...
    UnityEngine::AudioClip* Microphone::Start(StringW deviceName, bool loop, int lengthSec, int frequency) {
        ...
        if (microphoneDeviceIDFromName == -1)
            VRMLogger.error("Couldn't acquire device ID for device name %s", std::string(deviceName).c_str());
        if (lengthSec <= 0)
            VRMLogger.error("Length of recording must be greater than zero seconds (was: %d  seconds)", lengthSec);
        if (lengthSec > 3600)
            VRMLogger.error("Length of recording must be less than one hour (was: %d seconds", lengthSec);
        if (frequency <= 0)
            VRMLogger.error("Frequency of recording must be greater than zero (was: %d Hz)", frequency);
