...
float VRMQavatars::WristTwistFix::Dampen(float v, const float min, const float max)
{
...
void VRMQavatars::WristTwistFix::OnPostUpdate()
{
    if (wristWeight > 0.0f)
    {
...
void VRMQavatars::WristTwistFix::FixAxis(FixData* fixData, const float weight, const float dampen)
{
    if (fixData == nullptr)
    {
        return;
