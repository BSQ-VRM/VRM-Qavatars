...
namespace AssetLib
{
    class gLTFImageReader 
    {
        ...
        static ArrayW<uint8_t> ReadImageIndexRaw(const uint32_t jsonLength, std::ifstream& binFile, const int imageIndex)
        {
            //WHY DO PEOPLE DO THIS OH MY GOD IT IS NOT THAT HARD TO INCLUDE A THUMBNAIL AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            if(imageIndex == -1) return ArrayW<uint8_t>::Empty();
